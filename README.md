...
- раз в неделю, по понедельникам, проверяет `https://team.vk.company/vacancy/?specialty=287`;
...
- присылает один Telegram-дайджест со статистикой, ссылкой на историю и полным скриншотом страницы;
...
После этого проверка будет запускаться по понедельникам в `07:15 UTC`, то есть примерно в `10:15` по Москве.
...
- `data/snapshots/*.json` — снимки списка вакансий по каждому запуску.

## Что считается

- новые вакансии;
- новые вакансии за 7 дней;
- новые вакансии по месяцам;
- закрытые вакансии;
- закрытые вакансии за 7 дней;
